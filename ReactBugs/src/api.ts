...
export const bugs = {
  ...
  create(data: {
    ...
    return apiFetch('/bugs', { method: 'POST', body: JSON.stringify(data) })
  },
  update(
    id: string,
    data: Partial<{
    ...
    return apiFetch(`/bugs/${id}`, { method: 'PATCH', body: JSON.stringify(data) })
  },
  close(
    id: string,
...
export const products = {
  list(includeArchived = false): Promise<Product[]> {
    return apiFetch(`/api/products${includeArchived ? '?include_archived=true' : ''}`)
  },
  ...
  update(oldName: string, data: Partial<{ name: string; archived: boolean }>): Promise<Product> {
...
export const areas = {
  list(includeArchived = false): Promise<Area[]> {
    return apiFetch(`/api/areas${includeArchived ? '?include_archived=true' : ''}`)
  },
  ...
  update(oldName: string, data: Partial<{ name: string; archived: boolean }>): Promise<Area> {
...
export const severities = {
  list(includeArchived = false): Promise<Severity[]> {
    return apiFetch(`/api/severities${includeArchived ? '?include_archived=true' : ''}`)
  },
...
export const platforms = {
  list(includeArchived = false): Promise<Platform[]> {
    return apiFetch(`/api/platforms${includeArchived ? '?include_archived=true' : ''}`)
  },
...
export const agentsApi = {
  ...
  register(name: string, description?: string, rate_limit?: number): Promise<Agent> {
    return apiFetch('/agents', {
      method: 'POST',
...
export const authApi = {
  getQr(): Promise<{ qr_url: string; session_id: string }> {
    return apiFetch('/auth/qr')
  },
  pollSession(sessionId: string): Promise<{ token: string } | null> {
