...
def test_query_status_all(engine, backend):
    ...
    bugs_repo.close(engine, b["id"], resolution="fixed", actor="u", actor_type="human")
    result = backend.query({"status": "all"})
    ids = {r["id"] for r in result["bugs"]}
...
def test_query_fts_matches_title(engine, backend):
    ...
    result = backend.query({"q": "memo"})
    titles = [r["title"] for r in result["bugs"]]
...
def test_query_filters_product_and_priority(engine, backend):
    ...
    result = backend.query({"product": ["A"], "priority": [1]})
    titles = [r["title"] for r in result["bugs"]]
    assert titles == ["one"]
...
def test_query_has_artifacts_filter(engine, backend):
    ...
    with_art = [r["title"] for r in backend.query({"has_artifacts": True})["bugs"]]
    without_art = [r["title"] for r in backend.query({"has_artifacts": False})["bugs"]]
...
def test_query_related_to(engine, backend):
    ...
    result = backend.query({"related_to": a["id"]})
    assert [r["id"] for r in result["bugs"]] == [b["id"]]
...
def test_query_date_range(engine, backend):
    ...
    ts = bug["created_at"]
    result = backend.query({"created_after": ts, "created_before": ts})
    assert result["bugs"]
...
def test_index_remove_and_update_artifacts(engine, backend):
    ...
    result = backend.query({"q": "screenshot"})
    assert any(r["id"] == bug["id"] for r in result["bugs"])
